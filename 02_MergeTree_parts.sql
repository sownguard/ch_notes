...
SELECT
    number + rand(),
    floor(randUniform(1, 500000)),
    toString(user_id),
    user_id % 50,
    floor(randUniform(1, 20000)),
    car_id % 5,
    'Minute',
    today() - rand() % 365,
...
echo "INSERT INTO learn_db.async_carsharing SETTINGS async_insert=1 VALUES (1, now(), 'trip');" > query.sql
clickhouse-benchmark -i 1000000 -c 10 --query "`cat query.sql`"



-- recreate with partition
DROP TABLE IF EXISTS learn_db.mart_carsharing_trip;
...
(
    ...
    trip_cost Float32,
    load_date Date
    --INDEX idx_year trip_start_year TYPE minmax GRANULARITY 4
)
ENGINE = MergeTree()
PARTITION BY trip_start_year
ORDER BY (trip_start_date, user_id, trip_id);
